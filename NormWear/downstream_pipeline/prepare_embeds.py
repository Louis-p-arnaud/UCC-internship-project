...
def audio_embedding_prepare(data_rootpath="audio_downstream/Coswara", model_name='ast', root_prefix="../", remark="", args=None):
    ...
    os.makedirs('{}{}/{}_wav_embed'.format(root_prefix, data_rootpath, save_remark), exist_ok=True)

    # get embedding for each sample
    for fn in tqdm(sorted(os.listdir('{}{}/sample_for_downstream'.format(root_prefix, data_rootpath)))):
        # edge case
        if fn[0] == '.':
            continue
        ...
        # load sample
        # read data
        with open(os.path.join('{}{}/sample_for_downstream'.format(root_prefix, data_rootpath), fn), 'rb') as f:
        ...
        # save the embedding
        with open(os.path.join('{}{}/{}_wav_embed'.format(root_prefix, data_rootpath, save_remark), fn), 'wb') as f:
            pickle.dump({
                "uid": sample["uid"],
                "sampling_rate": sample['sampling_rate'],
...
def combine_normwear_ast(data_rootpath="audio_downstream/Coswara", root_prefix="../"):
    # initialize folder for save all the embedding
    os.makedirs('{}{}/nacombine_wav_embed'.format(root_prefix, data_rootpath), exist_ok=True)
    ...
    for fn in tqdm(sorted(os.listdir('{}{}/sample_for_downstream'.format(root_prefix, data_rootpath)))):
        ...
        # TODO combine embeds
        with open(os.path.join('{}{}/normwear_wav_embed'.format(root_prefix, data_rootpath), fn), 'rb') as f:
            normwear_embed = pickle.load(f)["embed"]
        with open(os.path.join('{}{}/ast_wav_embed'.format(root_prefix, data_rootpath), fn), 'rb') as f:
        ...
        # save the embedding
        with open(os.path.join('{}{}/nacombine_wav_embed'.format(root_prefix, data_rootpath), fn), 'wb') as f:
            pickle.dump({
                "uid": sample["uid"], 
                "sampling_rate": sample['sampling_rate'], 
                "embed": embed, # E*2
